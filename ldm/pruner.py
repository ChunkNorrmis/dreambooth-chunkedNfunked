def prune_checkpoint(checkpoint, dtype='float16'):
    precision = {'float16': lambda v: v.half().contiguous(), 'float32': lambda v: v.float().contiguous()}
    if int(checkpoint['global_step']) > 0:
        ...
        pruned_checkpoint['state_dict'] = {k: precision[dtype](v) for k, v in checkpoint['state_dict'].items()}
...
def prune_pickle(checkpoint, dtype='float16'):
    ...
    if int(checkpoint['global_step']) > 0:
        ...
        nil_pickle = {k: precision[dtype](v) for k, v in checkpoint['state_dict'].items()}
